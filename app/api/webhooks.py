...
def _copy_transcript_to_candidate(
    db: Session,
    booking: Booking,
    transcript: str,
    ...
    """
    EP17 — When a transcript arrives, if the booking has a linked candidate,
    copy the transcript to that candidate record and queue a re-embed so
    RYZE Intelligence can find them by what was said on the call.
    """
    if not booking.candidate_id:
        return

    try:
        candidate = (
            db.query(Candidate).filter(Candidate.id == booking.candidate_id).first()
        )
        if not candidate:
            logger.warning(
                f"[EP17] booking #{booking.id} has candidate_id={booking.candidate_id} "
                f"but no matching candidate row found"
            )
            return

        candidate.meeting_transcript = transcript
        db.commit()

        # Re-embed candidate so Intelligence can search by call content
        background_tasks.add_task(embed_candidate_background, candidate.id)

        logger.info(
            f"[EP17] Transcript copied to candidate #{candidate.id} ({candidate.name}) "
            f"from booking #{booking.id} — {len(transcript)} chars — re-embed queued"
        )
    except Exception as e:
        logger.error(
            f"[EP17] Failed to copy transcript to candidate for booking #{booking.id}: {e}"
        )


def _generate_summary_from_transcript(booking_id: int) -> None:
    """
    Generate summary/next_steps/keywords for a booking that has a transcript
    but no summary. Background-safe: opens its own session, since the request
    ...
    try:
        ...
        if not booking:
            logger.warning(f"[summary_from_transcript] Booking #{booking_id} not found")
            return
        if not booking.meeting_transcript:
            logger.info(f"[summary_from_transcript] Booking #{booking_id} has no transcript yet — skipping")
            return
        if booking.meeting_summary:
            logger.info(f"[summary_from_transcript] Booking #{booking_id} already has a summary — skipping")
            return
...
Booking type: {booking.booking_type}. Contact: {booking.employer_name}.
...
- "keywords": comma-separated skills/topics mentioned (string, or null)""",
        ...
        # Re-embed so the summary/keywords enrich the vector.
        embed_booking_background(booking.id)
        logger.info(f"[summary_from_transcript] Summary generated for booking #{booking_id}")
    except Exception as e:
        db.rollback()
        logger.error(f"[summary_from_transcript] Failed for booking #{booking_id}: {e}")
    finally:
        db.close()
...
async def zoom_webhook(
    ...
    if event == "recording.completed":
        ...
        if transcript:
            ...
            background_tasks.add_task(_generate_summary_from_transcript, booking.id)
    ...
    if event == "recording.transcript_completed":
        ...
        if transcript:
            ...
            # EP17 — copy transcript to linked candidate and re-embed
            _copy_transcript_to_candidate(db, booking, transcript, background_tasks)
            background_tasks.add_task(_generate_summary_from_transcript, booking.id)
